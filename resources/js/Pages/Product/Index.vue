<template>
	...
	<div class="pt-12 px-6">
		...
		<div class="flex mb-6 border-b border-slate-300">
			<TabsFilterByStatusLink :isActive="!route().params.status" :count="props.products_count.total" label="Todos"
				@click="filterByStatus(null)" />

			<TabsFilterByStatusLink :isActive="route().params.status === product_status_all.Active"
				:count="props.products_count.totalActive" label="Ativos" @click="filterByStatus(product_status_all.Active)" />

			<TabsFilterByStatusLink :isActive="route().params.status === product_status_all.Inactive"
				:count="props.products_count.totalInactive" label="Inativos"
				@click="filterByStatus(product_status_all.Inactive)" />

			<TabsFilterByStatusLink :isActive="route().params.status === product_status_all.Waiting"
				:count="props.products_count.totalWaiting" label="Aguardando"
		...
		<LayoutSection class="bg-white rounded-lg  divide-y">
			<div class="md:flex md:space-x-4 mb-4">
				<div class="w-1/3 relative flex-1">
					<FormInputText placeholder="Digite o nome do produto..." name="stock_virtual" id="stock_virtual" type="search"
			...
			<div
				...
				<span class="flex font-bold mx-3 cursor-pointer" @click="sort('name')">
					<span class="mr-1" :class="route().params.order_by !== 'name' ? 'hidden' : ''">
						<ArrowUp :class="route().params.direction === 'desc' ? 'rotate-180' : ''" />
					</span>
					Nome
				</span>

				<span class="flex font-bold cursor-pointer ml-auto mr-4" @click="sort('status')">
					<span class="mr-1" :class="route().params.order_by !== 'status' ? 'hidden' : ''">
						<ArrowUp :class="route().params.direction === 'desc' ? 'rotate-180' : ''" />
					</span>
					Status
				</span>
			...
			<div class="bg-white border-slate-200 overflow-hidden shadow-sm dark:bg-[#11183C] dark:border-slate-700"
				v-for="product in props.products.data" :key="product.id">
				<div class="flex items-center space-x-4 py-4 px-6">
					<!-- <input class="border-slate-300" type="checkbox" name="[]" /> -->

					<a href="#">
						<img class="w-16 h-16 object-fill rounded-full" :src="`https://picsum.photos/seed/${product.name
								.split(' ')
								.join('-')}/200/300`" alt="Produto" />
					</a>
					...
					<div class="flex items-end">
						<div class="flex items-center mr-4" :class="{
